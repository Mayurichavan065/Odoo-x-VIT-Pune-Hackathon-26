...
router.put('/:id/role', authenticateToken, requireRole('admin'), (req, res) => {
  ...
  const db = getDB();
  db.prepare('UPDATE users SET role = ? WHERE id = ? AND company_id = ?')
    .run(role, req.params.id, req.user.company_id);
  res.json({ success: true });
});

// PUT /api/users/:id/manager - assign manager (admin only)
router.put('/:id/manager', authenticateToken, requireRole('admin'), (req, res) => {
  const { manager_id } = req.body;
  const db = getDB();
  db.prepare('UPDATE users SET manager_id = ? WHERE id = ? AND company_id = ?')
