...
router.get('/pending', authenticateToken, requireRole('admin', 'manager'), (req, res) => {
  const db = getDB();
...
router.post('/:stepId/decide', authenticateToken, (req, res) => {
  ...
  if (step.status !== 'pending') return res.status(400).json({ error: 'Step already decided' });
  ...
  const transaction = db.transaction(() => {
    ...
    if (decision === 'rejected') {
      ...
      db.prepare("UPDATE expenses SET status = 'rejected', updated_at = datetime('now') WHERE id = ?").run(step.expense_id);
      // Skip remaining steps
      db.prepare("UPDATE approval_steps SET status = 'skipped' WHERE expense_id = ? AND step_order > ?")
        .run(step.expense_id, step.step_order);
    } else {
      // Check conditional rules
      evaluateConditionalRules(db, step.expense_id, step.company_id, step.total_amount);
      ...
      if (!nextStep) {
        db.prepare("UPDATE expenses SET status = 'approved', updated_at = datetime('now') WHERE id = ?").run(step.expense_id);
      }
    }
  });
...
function evaluateConditionalRules(db, expenseId, companyId, amount) {
  ...
  for (const rule of rules) {
    if (rule.rule_type === 'percentage' && rule.percentage_threshold) {
      const totalSteps = db.prepare('SELECT COUNT(*) as cnt FROM approval_steps WHERE expense_id = ?').get(expenseId);
      const approvedSteps = db.prepare("SELECT COUNT(*) as cnt FROM approval_steps WHERE expense_id = ? AND status = 'approved'").get(expenseId);
      const pct = (approvedSteps.cnt / totalSteps.cnt) * 100;
      if (pct >= rule.percentage_threshold) {
        db.prepare("UPDATE approval_steps SET status = 'skipped' WHERE expense_id = ? AND status = 'pending'").run(expenseId);
        db.prepare("UPDATE expenses SET status = 'approved', updated_at = datetime('now') WHERE id = ?").run(expenseId);
        return;
      }
    }

    if (rule.rule_type === 'hybrid' && rule.auto_approve_below && amount <= rule.auto_approve_below) {
      db.prepare("UPDATE approval_steps SET status = 'skipped' WHERE expense_id = ? AND status = 'pending'").run(expenseId);
      db.prepare("UPDATE expenses SET status = 'approved', updated_at = datetime('now') WHERE id = ?").run(expenseId);
      return;
    }
