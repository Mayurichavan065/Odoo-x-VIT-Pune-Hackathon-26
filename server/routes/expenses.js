...
router.post("/", authenticateToken, (req, res) => {
  ...
  if (!title || !category || !line_items || line_items.length === 0) {
    return res
      .status(400)
      .json({ error: "Title, category, and at least one line item required" });
  }
  ...
  const companyCurrency = db
    .prepare("SELECT base_currency FROM companies WHERE id = ?")
    .get(req.user.company_id);
  let converted_amount = total;
  if (
    currency &&
    companyCurrency &&
    currency !== companyCurrency.base_currency
  ) {
    const fromRate = db
      .prepare("SELECT rate_to_usd FROM currencies WHERE code = ?")
      .get(currency);
    const toRate = db
      .prepare("SELECT rate_to_usd FROM currencies WHERE code = ?")
      .get(companyCurrency.base_currency);
    if (fromRate && toRate) {
      converted_amount = (total / fromRate.rate_to_usd) * toRate.rate_to_usd;
    }
  }

  // Budget validation: check if the category has a monthly limit
  const budget = db
    .prepare(
  ...
  if (budget) {
    ...
    `,
      ...
      .get(req.user.company_id, category, currentMonth).val;

    const amountToCheck = converted_amount || total;
    if (spent + amountToCheck > budget.monthly_limit) {
      ...
      return res.status(400).json({
        ...
        details: {
          ...
          spent,
          remaining,
          requested: amountToCheck,
          over_by: spent + amountToCheck - budget.monthly_limit,
        },
      });
    }
  }
  ...
  const steps = db
    .prepare(
      "SELECT s.*, u.full_name as approver_name FROM approval_steps s JOIN users u ON s.approver_id = u.id WHERE s.expense_id = ? ORDER BY s.step_order",
    )
    .all(expenseId);

  res
    .status(201)
    .json({ ...expense, line_items: lines, approval_steps: steps });
});
...
function createApprovalWorkflow(db, expenseId, submitterId, companyId, amount) {
  ...
  if (autoRule) {
    db.prepare("UPDATE expenses SET status = 'approved' WHERE id = ?").run(
      expenseId,
    );
    return;
  }
  ...
  db.prepare("UPDATE expenses SET status = 'in_review' WHERE id = ?").run(
    expenseId,
  );
}
...
router.get("/", authenticateToken, (req, res) => {
  ...
  let query =
    "SELECT e.*, u.full_name as submitter_name FROM expenses e JOIN users u ON e.submitted_by = u.id WHERE e.submitted_by = ?";
...
router.get(
  ...
  (req, res) => {
    ...
    if (req.user.role === "admin") {
      query =
        "SELECT e.*, u.full_name as submitter_name FROM expenses e JOIN users u ON e.submitted_by = u.id WHERE e.company_id = ? ORDER BY e.created_at DESC";
...
router.put("/:id", authenticateToken, (req, res) => {
  ...
  const db = getDB();
  const expense = db
    .prepare("SELECT * FROM expenses WHERE id = ? AND submitted_by = ?")
    .get(req.params.id, req.user.id);
  if (!expense) return res.status(404).json({ error: "Expense not found" });
  if (!["draft", "rejected"].includes(expense.status)) {
    return res
      .status(400)
      .json({ error: "Cannot edit expense in current status" });
  }

  const total = line_items
    ? line_items.reduce((sum, item) => sum + (parseFloat(item.amount) || 0), 0)
    : expense.total_amount;

  const transaction = db.transaction(() => {
    db.prepare(
      "UPDATE expenses SET title = ?, description = ?, category = ?, currency = ?, total_amount = ?, updated_at = datetime('now') WHERE id = ?",
    ).run(
      title || expense.title,
    ...
    if (line_items) {
      db.prepare("DELETE FROM expense_lines WHERE expense_id = ?").run(
        req.params.id,
      );
      const insertLine = db.prepare(
        "INSERT INTO expense_lines (id, expense_id, description, amount, category, date) VALUES (?, ?, ?, ?, ?, ?)",
